...
def training_modules(pathway_file, gene_file, module_file_list, module_gene_file_list, meta_file, class_name, output_prefix, corr_threshold_list, batch_size, hidden_channels, learning_rate, epochs, sample_num, seed):
    torch.manual_seed(seed)
    random.seed(seed)
    np.random.seed(seed)
    if torch.cuda.is_available():
    ...
    f1 = open(module_file_list, 'r')
    list_module_file = [x.strip() for x in f1.readlines()]
    f1.close()
    f1 = open(module_gene_file_list, 'r')
    list_module_gene_file = [x.strip() for x in f1.readlines()]
    f1.close()
    f1 = open(corr_threshold_list, 'r')
    list_corr_threshold = [x.strip() for x in f1.readlines()]
    f1.close()

    module_dict_list = []
    for idx, module_file in enumerate(list_module_file):
        module_gene_file = list_module_gene_file[idx]
        corr_threshold = float(list_corr_threshold[idx])
    
        module_gene_corr = read_microbe_gene_corr(module_gene_file, 2)
        module_abundance = pd.read_csv(module_file, header=0, delimiter='\t', index_col=0)
    ...
    subset_expr, opgene = parse_gene_expression(gene_file, wpgene, sample_list)
    dataset5 = construct_pathnodes5(wpdict, subset_expr, module_dict_list)
    torch.save(dataset5, f'{output_prefix}_nodes.pt')
    label_df = pd.read_csv(meta_file, header=0, delimiter='\t', index_col=0)
    label_df['label'], _ = pd.factorize(label_df[class_name])
    dt4train5 = create_geometric_dataset(dataset5, wp_edge, label_df['label'])
    torch.save(dt4train5, f'{output_prefix}_training.pt')
    
    train_dataset, test_dataset = random_split(dt4train5, [int(0.8 * len(dt4train5)), int(0.2 * len(dt4train5))])
    train_dataset, val_dataset = random_split(train_dataset, [int(0.9 * len(train_dataset)), int(0.1 * len(train_dataset))])
    ...
    # Training, validation, and testing
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model = PathwayGAT(num_features=dataset5.shape[2], hidden_channels=hidden_channels, num_classes=len(set(label_df['label']))).to(device)
