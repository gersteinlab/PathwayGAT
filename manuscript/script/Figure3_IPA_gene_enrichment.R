library(ggplot2)
result <- read.table('files/JHU_COAD_final_gene_sum/JHU_COAD_diffexp_beta_IPA_pathway/JHU_COAD_diffexp_beta_35_IPA_pathway.txt', header = T, sep = '\t', quote = '', stringsAsFactors = F)
...
result <- result[grepl('Colon ', result$Diseases.or.Functions.Annotation) | grepl('Colore', result$Diseases.or.Functions.Annotation) | grepl('colore', result$Diseases.or.Functions.Annotation), ]
